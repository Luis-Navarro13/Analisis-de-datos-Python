...
LV1992_pre = mli.imread('3_9-10-1992.png')
LV2010_pre = mli.imread('5_8-11-2010.png')

# suavizado Gaussiano... luego veremos para que sirve
sigma = 8
LV1992fil_pre = gaussian_filter(LV1992_pre, sigma)
LV2010fil_pre = gaussian_filter(LV2010_pre, sigma)

fsize = 16
plt.figure()
plt.subplot(2,2,1)
plt.imshow(LV1992_pre)
plt.title('1992',fontsize=fsize)
plt.subplot(2,2,2)
plt.imshow(LV2010_pre)
plt.title('2010',fontsize=fsize)
plt.subplot(2,2,3)
plt.imshow(LV1992fil_pre)
plt.title('1992 filtrado',fontsize=fsize)
plt.subplot(2,2,4)
plt.imshow(LV2010fil_pre)
plt.title('2010 filtrado',fontsize=fsize)



LV1992 = DecompImage(LV1992_pre)
...
for jc in range(Ncolores):
 for jim in range(Nimages):
  if jim==0:
   imaux = LV1992; cmpaux = seq_cmap; limsaux=[0,1]   
  if jim==1:
   imaux = LV2010; cmpaux = seq_cmap; limsaux=[0,1]   
  if jim==2:
...
ciudadLV = ma.masked_greater(LV1992[:,:,0],0.4)

plt.figure()
plt.subplot(1,2,1)
plt.imshow(LV1992[:,:,0],vmin=0,vmax=1,cmap=seq_cmap)
plt.colorbar()
plt.subplot(1,2,2)
...
ciudadLV = ma.masked_greater(difagreg,0.2)

plt.figure()
plt.imshow(ciudadLV,vmin=-1,vmax=1,cmap=div_cmap)
plt.title('$d^{1/3}$')
plt.colorbar()
